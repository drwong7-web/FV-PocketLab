...
export async function generateDOCX(test: ExportTest): Promise<Blob> {
  ...
  const children: (Paragraph | Table)[] = [
    new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: test.type === "jump" ? "Profil F-V — Saut vertical" : "Profil F-V — Sprint linéaire", bold: true })] }),
    new Paragraph({ children: [new TextRun({ text: athleteName, bold: true, size: 28 })] }),
    new Paragraph({ children: [new TextRun({ text: [test.athlete.sport, new Date(test.test_date).toLocaleDateString("fr-FR"), test.athlete.body_mass ? `${test.athlete.body_mass} kg` : ""].filter(Boolean).join(" · "), italics: true, color: "666666" })] }),
    new Paragraph({ children: [new TextRun(" ")] }),
    new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Indicateurs principaux")] }),
  ];
  ...
  if (test.type === "jump") {
    ...
    children.push(new Table({
      ...
      rows: [
        row(["Indicateur", "Valeur", "Unité"], true, [3000, 3000, 3000]),
        ...
        row(["Pente F-V", r.slopeFV.toFixed(2), "(N/kg)/(m/s)"], false, [3000, 3000, 3000]),
        row(["Pente optimale", r.FVoptimal.toFixed(2), "(N/kg)/(m/s)"], false, [3000, 3000, 3000]),
        row(["FVimb", r.FVimbalance.toFixed(1), "%"], false, [3000, 3000, 3000]),
        row(["R²", r.r2.toFixed(3), ""], false, [3000, 3000, 3000]),
        row(["h max", (r.hMax * 100).toFixed(1), "cm"], false, [3000, 3000, 3000]),
        row(["Profil", r.profile.replace("_", " "), ""], false, [3000, 3000, 3000]),
      ],
    }));
    children.push(new Paragraph({ children: [new TextRun(" ")] }));
    children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Conditions de test")] }));
    children.push(new Paragraph({ children: [new TextRun(`Masse : ${raw.bodyMass ?? "—"} kg · hPO : ${raw.pushOffDistance ?? "—"} m`)] }));
    children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Essais")] }));
    const w = [1200, 2200, 2200, 1700, 1700];
    children.push(new Table({
      width: { size: 9000, type: WidthType.DXA },
      columnWidths: w,
      rows: [
        row(["#", "Charge add. (kg)", "Hauteur (cm)", "F (N/kg)", "V (m/s)"], true, w),
    ...
    children.push(new Paragraph({ children: [new TextRun(" ")] }));
    children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Conditions de test")] }));
    children.push(new Paragraph({ children: [new TextRun(`Masse : ${raw.bodyMass ?? "—"} kg · Taille : ${raw.height ?? "—"} m · Vent : ${raw.windSpeed ?? 0} m/s`)] }));
    children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Splits")] }));
    const w = [3000, 3000, 3000];
    children.push(new Table({
      width: { size: 9000, type: WidthType.DXA },
      columnWidths: w,
      rows: [
        row(["Distance (m)", "Temps mesuré (s)", "Temps modèle (s)"], true, w),
        ...r.splits.map((s) => row([
  ...
  if (test.chartPng && test.chartPng.byteLength > 0) {
    children.push(new Paragraph({ children: [new TextRun(" ")] }));
    children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Courbe Force-Vitesse")] }));
    children.push(new Paragraph({
      ...
      children: [new ImageRun({
        ...
        altText: { title: "Courbe F-V", description: "Profil force-vitesse", name: "fv-chart" },
      })],
    }));
  }
  ...
  children.push(new Table({
    ...
    columnWidths: w,
    rows: [
      row(["Exercice", "Séries × Reps", "Intensité"], true, w),
