...
import { Camera, Check, Plus, Ruler, Trash2, Zap } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {trials.map((tr, i) => (
            <div key={i} className="mt-2 grid grid-cols-[1fr_1fr_auto_auto] gap-2">
              ...
              <Button size="icon" variant="outline" onClick={() => setCameraIndex(i)} aria-label="Camera" disabled={!pxPerCm}>
                <Camera className="h-4 w-4 text-primary" />
              </Button>
          ...
          <p className="mt-2 text-xs text-muted-foreground">Tip: include an unloaded trial (load = 0) for h max prediction.</p>
        </CardContent>
      </Card>
