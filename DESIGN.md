...
Public first-visit page in `src/pages/Landing.tsx` + `src/components/landing/`. Same cockpit tokens: `glass-card`, `logo-well`, `engraved-logo`, `section-label`, `font-display`, `metric-value`. Brand marks: `fv-logo.png`, `logo-sprint-neon.png`, `logo-jump-neon.png`. Hero preview is two teaching F–V diagrams (`HeroFVProfiles.tsx`, SVG + tokens — not metric tiles and not a raster of a textbook figure). Motion (Tailwind, paused under `prefers-reduced-motion`): `animate-landing-letter` (hero lockup: FV logo then PocketLab letters left-to-right; `CenterFlickerTitle` on the hero title and section h2s, same opacity flicker staggered from the center out), `animate-landing-marquee` (audience chips), `animate-landing-reveal` (section enter via `useInView`), `animate-landing-word-squeeze-*` (`#features` heading: one-word carousel with horizontal scaleX squeeze and perspective; full title if reduced motion). Do not add a separate marketing palette or stock video. Prices live in `src/lib/landingPrices.ts`. Footer links go to `/privacy`, `/terms`, and `/contact` (`src/pages/Legal.tsx`) — same tokens, no extra palette.
